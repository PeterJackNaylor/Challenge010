...
class Model2(Model_pred):
    def retrain(self, list_img, dic, summary_train, output_csv):
        ...
        #var_to_init = [self.global_step, self.learning_rate, self.optimizer, self.training_op]
        # var_to_init = [var for var in tf.global_variables() if 'Adam' in var.name]
        # var_to_init = [] #[var for var in tf.global_variables() if 'optimizer' or 'Adam' in var.name]
        #var_to_init2 = [var for var in tf.global_variables() if 'ExponentialMovingAverage' in var.name]
        #var_to_init = var_to_init + [self.global_step] + var_to_init2
